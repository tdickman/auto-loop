...
from autoloop.models import Swap, print_all
...
def get_eligible_channels():
    ...
    * ...
    ...
    for channel in channels:
        ...
        if channel.local_balance / channel.capacity < config.LOOP_OUT_PERCENTAGE_TRIGGER:
            continue

        if (channel.local_balance - config.LOOP_OUT_AMOUNT) / channel.capacity < config.LOOP_OUT_MINIMUM:
            continue

        most_recent_swap = Swap.select().where(Swap.pub_key==channel.remote_pubkey).order_by(Swap.start_time.desc()).first()

        if config.DISABLE_LOOP_FOR_DUPLICATE_CHANNELS and channel_count[channel.remote_pubkey] > 1:
            continue
        ...
        if most_recent_swap:
            ...
            if most_recent_swap.state == 4:
                if most_recent_swap.start_time > get_epoch_nano() - config.DAYS_BETWEEN_RETRIES * NANO_SECONDS_DAY:
                    continue
...
def get_pending_loops():
    return Swap.select().where(Swap.state<3, Swap.start_time > get_epoch_nano() - config.MAX_AGE_DAYS * NANO_SECONDS_DAY)
...
def perform_loop_outs():
    ...
    for channel in get_eligible_channels():
        if remaining_loop_outs > 0:
            print('Looping out pubkey {}, channel {}'.format(channel.remote_pubkey, channel.chan_id))
